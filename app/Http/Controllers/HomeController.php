...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->priviliges_id == 3) {
            ...
            for ($i=0; $i < 7; $i++) { 
                $beratPerhari = Sampah::where('user_id',auth()->user()->id)->whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()])->sum('berat_sampah_total')->orderBy('created_at','DESC');
                $beratPerhariOrganik = Sampah::where('user_id',auth()->user()->id)->whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()])->sum('berat_sampah_organik')->orderBy('created_at','DESC');
                //$beratPerhariAnorganik = Sampah::where('user_id',auth()->user()->id)->whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()]);
        ...
        for ($i=0; $i < 7; $i++) { 
            $beratPerhari = Sampah::whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()])->sum('berat_sampah_total')->orderBy('created_at','DESC');
            $beratPerhariOrganik = Sampah::whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()])->sum('berat_sampah_organik')->orderBy('created_at','DESC');
            //$beratPerhariAnorganik = Sampah::where('user_id',auth()->user()->id)->whereBetween('created_at', [Carbon::now()->subDays($i)->startOfDay(),Carbon::now()->subDays($i)->endOfDay()]);
            $beratPerhariArray[] = $beratPerhari;
            //$totalBeratHariAnorganikArray[] = $totalBeratHariOrganik;
            $beratPerhariOrganikArray[] = $beratPerhariOrganik;
